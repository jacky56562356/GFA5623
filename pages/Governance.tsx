        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          <aside className="lg:col-span-4 space-y-3">
            <div className="sticky top-32 bg-white p-12 rounded-[40px] shadow-2xl border border-gfa-border animate-fade-up">
              ...
              <div className="mt-3 pt-5 border-t border-gfa-border">
                <ul className="space-y-3 text-[10px] font-bold uppercase tracking-[0.3em] text-gfa-slate">
          ...
          <main className="lg:col-span-8 space-y-4">
            ...
            <section className="animate-fade-up">
              <h2 className="text-4xl font-bold font-serif mb-3 text-gfa-inkBlack leading-tight">{gov.summaryTitle}</h2>
            ...
            <section className="animate-fade-up delay-200">
              <h2 className="text-4xl font-bold font-serif mb-3 text-gfa-inkBlack leading-tight">{gov.roleTitle}</h2>
              <p className="text-xl text-gfa-slate mb-5 leading-relaxed font-light font-serif italic opacity-80">
                {gov.roleBody}
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {gov.board.map((member: any, i: number) => (
                  <div key={i} className="bg-white p-10 rounded-[32px] border border-gfa-border shadow-xl hover:shadow-2xl hover:border-gfa-gold transition-all group">
                    <h4 className="text-[10px] font-bold uppercase tracking-[0.3em] text-gfa-gold mb-3 opacity-60 group-hover:opacity-100 transition-opacity">{member.role}</h4>
                    <p className="text-2xl font-bold font-serif text-gfa-inkBlack leading-tight">{member.title}</p>
                    <div className="mt-4 h-1 w-16 bg-gfa-border group-hover:w-full transition-all duration-1000"></div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="bg-gfa-inkBlack text-white p-16 md:p-24 rounded-[48px] relative overflow-hidden shadow-2xl animate-fade-up delay-300">
              ...
              <p className="text-xl leading-relaxed mb-5 opacity-70 font-light italic font-serif"> The GFA Charter is a voluntary commitment. Participation signals an organization's dedication to professional safety and structural transparency without replacing statutory legal obligations. </p>
