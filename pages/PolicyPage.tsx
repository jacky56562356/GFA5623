    ...
    if (type === 'privacy') {
      return {
        ...
        sections: isEn ? {
          ...
          "3. Cookie 和定向广告 (Google Ads & Analytics)": "我们使用 Google Analytics 分析网站流量并使用 Google Ads 进行广告推广。包括 Google 在内的第三方供应商使用 Cookie 来根据您过去对我们网站的访问投放广告（再营销）。Google 及其合作伙伴使用广告 Cookie，使其能够根据您对我们网站和/互联网上其他网站的访问向您提供广告。您可以访问 Google 广告设置 (adssettings.google.com) 选择停用个性化广告。",
          "4. 第三方链接及供应商": "我们的网站可能包含由第三方供应商提供的广告。您可以访问 www.aboutads.info 或 optout.networkadvertising.org 选择停用部分第三方供应商对个性化广告 Cookie 的使用。我们的网站也可能包含指向其他网站的链接。我们对这些其他网站的隐私惯例不承担责任。",
      ...
      return {
        title: isEn ? "Cookie Policy" : "Cookie政策",
        summary: isEn 
          ? "This Cookie Policy explains how Global Film Alliance uses cookies and similar technologies to recognize you when you visit our website."
          : "本Cookie政策解释了全球电影联盟在我们网站上使用Cookie和类似技术来识别您的情形。",
        sections: isEn ? {
          ...
          "1. 什么是Cookie？": "Cookie 是您访问网站时放置在您的计算机或移动设备上的小数据文件。网站所有者广泛使用 Cookie 以使其网站正常运行或更高效地运行，并提供报告信息。",
          "2. 我们为何使用Cookie？": "我们出于多种原因使用第一方和第三方 Cookie。我们的网站运行需要一些因技术原因不可或缺的 Cookie，我们将这些 Cookie 称为“必需”或“绝对必要”Cookie。其他 Cookie（包括来自 Google Analytics 和 Google Ads 的 Cookie）还允许我们跟踪和定位用户的兴趣，以增强您在我们网站上的体验并提供个性化广告（再营销）。",
          "3. 如何管理Cookie？": "您有权决定接受还是拒绝 Cookie。您可以通过在网络浏览器中设置您的首选项来行使您的 Cookie 权利。您还可以通过访问 Google 广告设置 (adssettings.google.com) 或 www.aboutads.info 选择停用个性化广告。如果您选择拒绝 Cookie，您仍然可以使用我们的网站，尽管您可能会受到网站某些功能和区域的限制。"
        }
      };
    }
